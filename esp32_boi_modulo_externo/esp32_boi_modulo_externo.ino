/*
  ...
  VERSÃO: MODULO TTP223 EXTERNO + mDNS + TELEGRAM + BOTOES EXTRAS
*/
...
<h2>Painel do Boi (Modulo Externo)</h2>
...
void loop() {
  if (mp3->isRunning()) { 
    ...
    // Checa mensagens do Telegram apenas quando não estiver tocando som
  ...
  if (licenciado && !bloqueado) {
    ...
    // Leitura DIGITAL do módulo externo TTP223
    if (digitalRead(TOUCH1_PIN) == HIGH && millis() - ultimoTouch1 > DEBOUNCE_MS) {
      ultimoTouch1 = millis();
      if (trackTouch1 != "") { ligarAmpliETocar(trackTouch1); }
    }

    if (digitalRead(TOUCH2_PIN) == HIGH && millis() - ultimoTouch2 > DEBOUNCE_MS) {
      ultimoTouch2 = millis();
